...
public:
	...
	/// <summary>
	/// calculates the total potential difference of a moved charge
	/// </summary>
	/// <param name="W">The work done.</param>
	/// <param name="q">The charge.</param>
	/// <returns>the potential difference of moved charge</returns>
	static ld potentialDifference_MovedCharge_a2b(const ld W, const ld q);
	...
	/// <summary>
	/// Will the electric field strength between two parallel conducting
	/// plates exceed the breakdown strength of Emax dialectic strength
	/// if the plates have a potential difference of V applied?
	/// How close together can the plates be with this applied voltage?
	/// </summary>
	/// <param name="V">The voltage.</param>
	/// <param name="Emax">The dielectric strength of material.</param>
	/// <returns>the closest the plates could be</returns>
	static ld distanceMaxBetweenPlateConductors(const ld V, const ld Emax = dielectric_strength.Air);
	...
	/// <summary>
	/// Calculates what voltage is applied to a capacitor of C
	/// farads that stores E Joules of energy? reworked equation Ecap = (CV^2)/2
	/// and solved for V.
	/// </summary>
	/// <param name="E">The stored energy in Joules.</param>
	/// <param name="C">The capacitance.</param>
	/// <returns>the voltage</returns>
	static ld voltageAppliedToCapacitorWithStoredEnergy(const ld E, const ld C);

	/// <summary>
	/// Calculates the amount of stored charge.
	/// </summary>
	/// <param name="C">The Capacitance.</param>
	/// <param name="E">The stored energy in Joules.</param>
	/// <returns>the charge q(C)S</returns>
	static ld chargeStoredOnCapacitor(const ld C, const ld E);

	/// <summary>
	/// Calculates the capacitance of a capacitor holding a stored charge of
	/// E Joules when a voltage of V is applied across it.
	/// </summary>
	/// <param name="E">The stored charge energy in Joules.</param>
	/// <param name="V">The voltage.</param>
	/// <returns>the capacitance in Farads</returns>
	static ld capacitanceOfCapacitor(const ld E, const ld V);

	/// <summary>
	/// calculates the max energy a parallel plate capacitor can store for a
	/// given dielectric material when the maximum energy is directly
	/// proportional to the volume of dielectric ( Volume=A⋅d). Note that the
	/// applied voltage is limited by the dielectric strength.
	/// </summary>
	/// <param name="k">The dielectric constant.</param>
	/// <param name="A">The area of plate.</param>
	/// <param name="ds">The dielectric strength of material used.</param>
	/// <param name="d">The distance between plates.</param>
	/// <returns>the Emax of the parallel plate capacitor</returns>
	static ld energyMaxParallelPlateCanStoreWithDielectric(
		const ld k, const ld A, const ld ds, const ld d);
...
inline ld ElectricPotential::potentialDifference_MovedCharge_a2b(const ld W, const ld q)
{
	return W / q;//
}
...
inline ld ElectricPotential::distanceMaxBetweenPlateConductors(const ld V, const ld Emax)
{
	return V / Emax;
}
...
inline ld ElectricPotential::voltageAppliedToCapacitorWithStoredEnergy(const ld E, const ld C)
{
	return sqrt((2.0 * E) / C);
}


inline ld ElectricPotential::chargeStoredOnCapacitor(const ld C, const ld E)
{
	return sqrt(2.0 * C * E);
}

inline ld ElectricPotential::capacitanceOfCapacitor(const ld E, const ld V)
{
	return (2.0 * E) / (V * V);
}

inline ld ElectricPotential::energyMaxParallelPlateCanStoreWithDielectric(const ld k, const ld A, const ld ds, const ld d)
{
	return (k * _E0_ * A * (ds * ds) * d) / (2.0);
}
